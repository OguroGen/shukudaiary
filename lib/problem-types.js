...
export function getParameters(homework) {
  ...
  // 旧カラムからparametersを生成（後方互換性）
  const parameters = {}
  for (let i = 1; i <= 10; i++) {
    parameters[`parameter${i}`] = null
  }
  
  if (homework.type === 'mitori') {
    parameters.parameter1 = homework.left_digits
    parameters.parameter2 = homework.rows
  } else if (homework.type === 'mul') {
    parameters.parameter1 = homework.left_digits
    parameters.parameter2 = homework.right_digits
  } else if (homework.type === 'div') {
    // 割り算は除数と商の順番
    parameters.parameter1 = homework.right_digits // 除数
    parameters.parameter2 = homework.left_digits // 商
  }
